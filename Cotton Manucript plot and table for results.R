...
write.csv(Yield_Summary_table, "Cotton_Yield_summary.csv")
write.csv(NUE_Summary_table, "Cotton_NUE_summary.csv")
write.csv(WUE_Summary_table, "Cotton_WUE_summary.csv")
...
nue_plot_Cotton_2021<-NUE_plot_dat %>% 
  ...
  labs(title="NUE (2021)",
       # x="Nitrogen Application (lb/acre)",
            x=element_blank(),
       y = "NUE (%)",
...
WUE_plot_Cotton_2021<-WUE_plot_dat %>% 
  ...
  guides(colour = guide_legend(override.aes = list(size=5)))+
  labs(title="WUE (2021)",
       x="Nitrogen Application (lb/acre)", y = "WUE (bu/ac-in)",
       color=('Irrigation Treatment'),
       # shape=('Irrigation Treatment'),
       # linetype=('Irrigation Treatment')
  )
...
WUE_plot_Cotton_2020<-WUE_plot_dat %>% 
  ...
  theme_bw()+
  # scale_x_continuous(breaks=seq(80, 240, 80))+
  scale_y_continuous(limits = c(-200,100), breaks=seq(-200, 100, 100))+
  theme(plot.title = element_text(size=14),
        legend.position="top",
        legend.title=element_blank(),
        legend.box = "horizontal",
...
ggp_all <- annotate_figure(p, top = text_grob("Cotton results"
               , face = "bold", size = 18))
ggp_all
 ggsave(paste0(plot_loc,"Interactionplot_Cotton.png"),ggp_all, width = 12, height = 12, units="in", dpi = 600)
